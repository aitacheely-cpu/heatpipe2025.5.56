...
M_charge = 6; % [kg] 系统初始充注量

%% 2. 求解器初始猜测值与边界设置
X0 = [2, 1.5, 0.1];        % [P0(MPa), H0(m), W(kg/s)]
...
options = optimoptions('lsqnonlin', 'Display', 'off', 'Algorithm', 'trust-region-reflective', ...
    'FunctionTolerance', 1e-6, 'StepTolerance', 1e-8, 'MaxIterations', 500);
...
% 【新增】提取并展示详细残差分量
F_final = TPTL_residuals(X_sol, geo, M_charge);
fprintf('\n=== 全局求解完成 ===\n');
...
for i = 1:length(solutions)
    ...
    if err_i < error_threshold
        ...
        fprintf('解 %2d: P0=%.4f, H0=%.4f, W=%.4f | 残差=%.2e\n', i, x_i(1), x_i(2), x_i(3), err_i);
    end
end
...
%% ===================== 局部函数区 (TPTL_residuals 等保持不变) =====================
% ... (以下内容保持你提供的原代码中的函数部分不变)
%% ===================== 局部函数区 =====================
