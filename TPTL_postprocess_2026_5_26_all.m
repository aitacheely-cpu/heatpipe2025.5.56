...
% =========================================================================
    ...
    % ----------------------------- 冷凝段水平 (3并) 无重力 -----------------------------
    [~, ~, ~, x_th, ~, ~, ~] = get_fluid_state(P_curr, h_curr); 
    dP_local_bend4 = calc_local_dp(P_curr, x_th, G_riser, geo.K_bend); P_curr = P_curr - dP_local_bend4 / 1e6;
    ...
    fprintf('\n====== 3. 各弯头局部阻力压降统计 ======\n');
    fprintf('下落至水平弯头局部压降 = %8.2f Pa\n', dP_local_bend1);
    fprintf('分流集管入口局部压降   = %8.2f Pa\n', dP_local_bend2);
    fprintf('汇流至上升段局部压降   = %8.2f Pa\n', dP_local_bend3);
    fprintf('冷凝分流集管局部压降   = %8.2f Pa\n', dP_local_bend4);
    fprintf('----------------------------------------\n');
    ...
    % 修复 M-Lint 警告：在绘图文本中显示出计算的 Q_avg
